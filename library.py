...
class Garden:                                 # Класс садовый участок
    ...
    def plant_cultivated_plant(self, cultivated_plant_name: str, num: int) -> None:  # Посадить культурное растение (тут в грядку)
        if self.__money < 20:
            print('Не хватает денег')
    ...
    def show(self) -> None:
        print("Day: {}".format(self.__day))
        print("Weather: {}".format(self.__weather))
        print("Money: {}".format(self.__money))
        ...
        for tree in self.__trees:
            ...
            if tree.check_harvest():
                ...
                temp_str = temp_str + "Harvest-|"
            print("Tree {}".format(counter) + "|" + temp_str)
            counter = counter + 1
        print("=" * 90)
        counter = 1
        for bed in self.__garden_bed:
